...
const SAPI_SEARCH_PATH = 'http://api.ft.com/content/search/v1';
const SAPI_PATH = 'http://api.ft.com/enrichedcontent';
...
function search(params) {
	const sapiUrl = `${SAPI_SEARCH_PATH}?apiKey=${CAPI_KEY}`;
...
function getContent(url, params={}) {
	...
	return fetchResText(url, options).then(response => {
		let sapiObj;
		try {
		 	sapiObj = JSON.parse(response);
		}
		catch( err ){
			throw new Error(`JSON.parse: err=${err},
				text=${response},
	...
	.catch( err => {
		console.log(`ERROR: getContentByID: err=${err}.`);
		return { params }; // NB, no sapiObj...
	})
}

function getContentByID(id, params={}) {
	const capiUrl = `${SAPI_PATH}/${id}?apiKey=${CAPI_KEY}`;
	return getContent(capiUrl, params);
}

function getContentByApiUrl(apiUrl, params={}) {
	const capiUrl = `${apiUrl}?apiKey=${CAPI_KEY}`;	
	return getContent(capiUrl, params);
}

function searchBetweenRange(afterIsotime, beforeIsotime, word=null, params={}) {
