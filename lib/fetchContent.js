...
const SAPI_SEARCH_PATH = 'https://api.ft.com/content/search/v1';
const SAPI_PATH = 'https://api.ft.com/content';
...
function getContentByID(id, params={}) {
	console.log(id);
	const sapiUrl = `${SAPI_PATH}/${id}/?apiKey=${CAPI_KEY}`;
	const options = {
		method: 'GET',
		headers: {
			'Content-Type': 'application/json'
	...
	return fetchResText(sapiUrl, options).then(response => {
		let sapiObj;
		try {
